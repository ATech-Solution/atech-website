// Portfolio Hero — Figma node 1:29121
// White background, centered badge + heading + subheading + 2 CTAs + 4-stat row
...
// ─── PortfolioHeroBlock ───────────────────────────────────────────────────────
export default function PortfolioHeroBlock({ data }: { data: PortfolioHeroData }) {
  const { badge, badgeIconSrc, heading, subheading, cta, stats } = data
  ...
  return (
    <section className="px-6 md:px-10 py-24" style={{ background: '#ffffff' }}>
      <div className="mx-auto flex flex-col items-center text-center gap-6" style={{ maxWidth: '896px' }}>
        ...
        {/* Stats row */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 w-full mt-8 pt-8" style={{ borderTop: '1px solid #e5e5e5' }}>
          {stats.map((stat) => (
            <div key={stat.label} className="flex flex-col items-center gap-1">
              <span style={{ fontFamily: 'var(--font-work-sans, sans-serif)', fontSize: '1.875rem', fontWeight: 400, color: '#171717', lineHeight: '36px' }}>
                {stat.value}
              </span>
              <span className="text-center" style={{ fontFamily: 'var(--font-work-sans, sans-serif)', fontSize: '0.75rem', color: '#737373', lineHeight: '16px' }}>
                {stat.label}
              </span>
            </div>
          ))}
