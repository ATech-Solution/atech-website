// Get Involved Quote Form — Figma node 1:30513
// White background, centered heading + large quote form

// ─── Types ────────────────────────────────────────────────────────────────────
interface GetInvolvedQuoteData {
  heading:     string
  subheading:  string
  submitLabel: string
}

// ─── GetInvolvedQuoteBlock ────────────────────────────────────────────────────
export default function GetInvolvedQuoteBlock({ data }: { data: GetInvolvedQuoteData }) {
  const { heading, subheading, submitLabel } = data
    ...
    >
      <div className="mx-auto flex flex-col items-center gap-6" style={{ maxWidth: '896px' }}>
        ...
        <form className="w-full flex flex-col gap-6 mt-4" style={{ maxWidth: '672px' }}>
          ...
          {/* Selects */}
          <select
            className="w-full px-6 py-4 text-base outline-none appearance-none"
            style={{
              border:     '2px solid #e5e5e5',
              background: '#efefef',
              fontFamily: 'var(--font-work-sans, sans-serif)',
          ...
          <textarea
            ...
            rows={5}
            className="w-full px-6 py-4 text-base outline-none resize-none"
            style={{
              border:     '2px solid #e5e5e5',
              background: '#ffffff',
              fontFamily: 'var(--font-work-sans, sans-serif)',
