...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`ALTER TABLE \`blocks\` ADD \`survey_form_ref_id\` integer REFERENCES forms(id);`)
  await db.run(sql`CREATE INDEX \`blocks_survey_form_ref_idx\` ON \`blocks\` (\`survey_form_ref_id\`);`)
}
